...
<script language="JavaScript">
										...
										function() {
											...
											if (confirm(msg) == true) {
												...
												if (becheckbox.length > 0) //如果获取到
												{
													...
													window.location = "deletecourse.action?checkTnum="
															+ becheckbox;
												} else {
													alert("请选择一行数据");
													return false;
													...
													.open({
														...
														yes : function(index,
															...
															var credit = $(
																	"#credit")
																	.val();
															window.location.href = "addcourse.action?cid="
																	+ cid
																	+ "&cname="
																	+ cname
																	+ "&property="
						...
						$("#searchbutton")
								.click(
										function() {
											...
											window.location.href = "searchcourse.action?sort="
													+ sort
													+ "&menuname="
													+ menuname;
										});
	...
	function revise() {
		...
		var property = $("#t1 tr:eq('" + rowIndex + "') td:eq(3)").text();
		var credit = $("#t1 tr:eq('" + rowIndex + "') td:eq(4)").text();
		$("#cid").val(cid);
		$("#cname").val(cname);
		...
		layer.open({
			...
			yes : function(index, layero) {
				...
				window.location.href = "savecourse.action?cid=" + cid
						+ "&cname=" + cname + "&property=" + property
						+ "&credit=" + credit;
				layer.close(index); //如果设定了yes回调，需进行手工关闭
			}
		});
			...
			<tr>
				...
				<td align="right">
					<div class="result-list">
						<a id="addcour" class="layui-btn" href='javascript:void(0)'> <i
							class="layui-icon">&#xe608;</i>新增课程 </a> <a id="batchDel"
	...
	<div id="addcourse" style="display:none">
			...
			<div class="layui-form-item">
				<div class="layui-inline">
					...
					<div class="layui-input-inline">
						<select id="property" name="modules" lay-verify="required"
							lay-search="">
		...
		<div class="layui-input-block">
			<input type="text" name="credit" id="credit" required
				lay-verify="required" placeholder="请输入学分" autocomplete="off"
				class="layui-input">
		</div>
	</div>
</body>
